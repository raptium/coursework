...
class mediaServer extends Thread{
    ...
    Hashtable<String, String> m_mime;
    ...
    public void run(){
        ...
        try{
            ...
            try {
                System.out.println(media_dir + path.substring(1));
        ...
        try{
            ...
            while(true) {
                if(iStream.available() < 1024) { 
                    byte[] sbuf = new byte[1024];
                    iStream.read(sbuf);
                    m_server.sendData(m_channel, sbuf);
                    break;
                }
                else {
                    iStream.read(buffer); 
                    m_server.sendData(m_channel, buffer);
                    sent ++;
